...
```{r}
# analyze variables and the way that they are distributed through histograms. 
...
```
```{r}
# create histogram plots for variables to understand distribution
...
```
```{r}
# understand relationship between covariates and predictor variable which is rings. use ggplot for plots to understand for further analysis
# when forming models 
...
```{r}
#Backwards elimination of features that do not provide significance to the model and refitting the data. Looking at the linear regression model fit, we can work to eliminate features that are not found as significant and see if those values change the outcome of the model. 

# update the model and get rid of features that are insignificant to the model
...
```{r}
#use a basic log-linear regression model to model the difference between linear regression versus log-linear model prediction. The reason we chose log-linear is due to the fact the number of rings can also be considered as a count variable. When we try to fit a linear regression onto the model, we end up with a skewed and overfitted representation of the predicted data.
...
```{r}
#third random forest model w number of trees being 80 and plot MSE over ntrees, this might either improve or overfit the model. 
